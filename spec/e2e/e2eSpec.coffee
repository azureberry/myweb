describe 'テスト（更新履歴）', ->
  ...
  getScrollTop = -> browser.executeScript('return document.body.scrollTop;')
  ...
  it 'サブメニューリンクをクリックすることで、ページ内リンク先に移動すること', ->
    ...
    # クリック後のスクロール位置の確認
    submenu_link_last.$('a').getAttribute('href').then (submenu_link_url) ->
      submenu_link_id = getLinkId(submenu_link_url)
      $(submenu_link_id).getLocation().then (locationdata) ->
        expect(getScrollTop()).toEqual locationdata.y - 70
  ...
  it 'スクロールすると、サブメニューが合わせて移動すること', ->
    ...
    getScrollTop().then (scrollTop) ->
      submenu.getLocation().then (locationdata) ->
        expect(locationdata.y).toEqual scrollTop + init_locationdata_y

  it 'スクロールすると、scrollspyが動作すること', ->
    # 初期はactive状態ではないこと
    expect(hasClass(submenu_link_list.get(SCROLL_TEST_ITEM), 'active')).toBe(false)
    ...
    expect(hasClass(submenu_link_list.get(SCROLL_TEST_ITEM), 'active')).toBe(true)
